...
- 策略研究员：新策略管理、策略 DSL、策略回测、因子进化、回测结果分析。
- 交易执行用户：实时监控、虚拟仓、实盘仓、自动交易、风控熔断和收益对照。
...
- 实盘安全边界：实盘交易必须账户可用、显式启用、风控通过、全程事件审计；当前不再保留手工确认队列。
...
| 新策略管理 | `/strategies` | 策略列表、模板、DSL、版本和编译 |
| 策略回测 | `/backtest` | 回测创建、进度、记录、对比和结果入口 |
| 实时监控 | `/realtime` | 多周期监控实例、事件流、风控、自动交易和策略收益对照 |
...
- 资讯之眼：资讯入库表为 `market_news_items`，后台 worker 会周期性抓取交易所/巨潮公告、东方财富公告/研报、个股新闻和 7x24 快讯。
- 扩展数据：筹码、资金流、财务快照和研报任务已纳入回测数据任务链路，分别落到 `stock_chip_distribution`、`stock_money_flow`、`stock_financial_snapshots` 和 `market_news_items`。
...
- 实盘仓允许在账户可用、交易开关开启、风控通过时提交受控委托；账户不可用或 QMT 仅快照可用时禁止提交。
- 虚拟仓可用于 QMT 模拟交易联调，虚拟仓账户状态必须独立展示为可用/不可用。
